...
public class RunExperiment extends Thread {
	...
	public void run() {
		...
		outputRuntimeFile_ = (String) map_.get("outputRuntimeFile");
		noOfObjectives_ = (Integer) map_.get("noOfObjectives");
...
//						problemList_[problemId];
				...
				System.out.println(Thread.currentThread().getName() + " Running algorithm: " + 
						...
						", run: " + runs);
				long initTime,estimatedTime = 0;
				try {
					try {
						initTime = System.currentTimeMillis();	   // Start the algorithm 
						resultFront= algorithm[alg].execute();				      
						estimatedTime = System.currentTimeMillis() - initTime;//End of the algorithm
				...
				this.writeDataToFile(directory + "/" + outputRuntimeFile_ + "." + runs, estimatedTime);
				
				// Record the number of feasible solutions
		        int feasibleNo = 0;        
		        for (int i=0;i<resultFront.size();i++) {
		        	if (resultFront.get(i).getOverallConstraintViolation() == 0.0) {
		        		feasibleNo ++;
		        	}
		        }	       
		        
				this.writeDataToFile(directory + "/FS" + "." + runs, feasibleNo); // The number of feasible solutions
