...
public class MetricsUtil {
  ...
  /** 
   *  This method returns the Pareto distance between
   *  two points given as <code>double []</code>
   *  @param a A reference point
   *  @param b A front point
   *  @return The euclidean distance between the points
   **/
  public double ParetoDistance(double [] a, double [] b) { 
    double distance = 0.0; 
    
    for (int i = 0; i < a.length; i++) {
    	// For minimization problem, b[i]-a[i], a[i] is reference point
    	// For maximization problem, a[i]-b[i], a[i] is reference point
  ...
  /**
   * Gets the Pareto distance (used in IGD+) between a point and the nearest one in
   * a given front (the front is given as <code>double [][]</code>)
   * @param point The point
   * @param front The front that contains the other points to calculate the 
   * distances
   * @return The minimun distance between the point and the front
   **/
  public double ParetoDistanceToClosedPoint(double [] point, double [][] front) {
    double minDistance = ParetoDistance(point,front[0]);
    
    
    for (int i = 1; i < front.length; i++) {
      double aux = ParetoDistance(point,front[i]);
      if (aux < minDistance) {
        minDistance = aux;
      }
    }
  ...
  public SolutionSet readNonDominatedSolutionSet(String path) {
	...
	public void readNonDominatedSolutionSet(String path, NonDominatedSolutionList solutionSet) {
		try {
			...
			String aux = br.readLine();
			while (aux!= null) {
				if(PseudoRandom.randDouble(0, 1) < 1.0){// ����Ϊ�˼���PF�ĸ���
					StringTokenizer st = new StringTokenizer(aux);
					int i = 0;
					Solution solution = new Solution(st.countTokens());
	 ...
	 * and store it in a existing non dominated solution set
	 * @param path The path of the file containing the data
	 * @param variablePath The path containing the variables
	 * @return A solution set
	 */
	public void readNonDominatedSolutionSet(String path,String path_var, NonDominatedSolutionList solutionSet) {
		try {
			/* Open the file */
			FileInputStream fis   = new FileInputStream(path)     ;
			InputStreamReader isr = new InputStreamReader(fis)    ;
			...
			/* Open the file */
			FileInputStream fis_var   = new FileInputStream(path_var)     ;
			InputStreamReader isr_var = new InputStreamReader(fis_var)    ;
			BufferedReader br_var      = new BufferedReader(isr_var)      ;	      	     
			
			
			String aux = br.readLine();
			
			String aux_var = br_var.readLine();			
			
			while (aux!= null) {
					StringTokenizer st = new StringTokenizer(aux);					
					
					StringTokenizer st_var = new StringTokenizer(aux_var);		
					
					Solution solution = new Solution(st.countTokens(),st_var.countTokens());
					...
					if (solutionSet.add(solution)) {
						int i_var = 0;
						while (st_var.hasMoreTokens()) {
							double value = new Double(st_var.nextToken());
							solution.getDecisionVariables()[i_var].setValue(value);
							i_var++;
						}
					}

				aux = br.readLine();
				aux_var = br_var.readLine();
			}
			br.close();
			br_var.close();
		} catch (Exception e) {
	   ...
	   */
	   public int chekDominance(double [] object1, double [] object2) {
	     if (object1==null)
	       return 1;
	     else if (object2 == null)
	       return -1;	     

	     int dominate1 ; // dominate1 indicates if some objective of solution1 dominates the same objective in solution2. dominate2
	...
	private int length(double[] object1) {
